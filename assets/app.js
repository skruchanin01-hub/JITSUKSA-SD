...
function loadCurrentChapter(){
  ...
  $('chantStage').scrollTop=0; state.ignoreScrollUntil=Date.now()+700; $('liveTotalScore').textContent=Math.round(state.totalScore);
...
function audioLoop(){
  ...
  if(state.audio.micOn){
    ...
    if(active){ state.audio.activeFrames++; state.audio.combo++; state.audio.bestCombo=Math.max(state.audio.bestCombo,state.audio.combo); const diff=Math.abs(rms-state.audio.lastRms); state.audio.stabilitySum += Math.max(0,100-diff*5); if(state.audio.combo>0 && state.audio.combo%Number(state.settings.comboRgbEvery||6)===0) triggerRgb(); $('micStateText').textContent=rms>threshold*2.4?'เสียงดีมาก':'เสียงชัดเจน'; }
    else { state.audio.combo=0; $('micStateText').textContent=rawActive?'กำลังจับเสียง...':'เสียงยังไม่ชัด'; }
    state.audio.lastRms=rms; const chapterScore=calculateCurrentChapterScore(); state.audio.score=chapterScore; $('liveTotalScore').textContent=Math.round(state.totalScore+chapterScore); $('comboText').textContent=`คอมโบ x${state.audio.combo}`;
  }
